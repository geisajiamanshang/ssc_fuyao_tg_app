...
|---|---|---|---|---|
| 测试 | `test` | `/root/ssc_fuyao_tg_app_test` | `ssc-offer-bot-test` | `测试1` / `测试2` / `测试3` / `测试4` / `测试4.1` / `测试11` / `测试21` / `测试6` / `测试6.1` / `测试6.2` / `测试7` / `测试5` |
| 生产 | `main` | `/root/ssc_fuyao_tg_app_prod` | `ssc-offer-bot-prod` | `1` / `2` / `3` / `4` / `4.1` / `11` / `21` / `6` / `6.1` / `6.2` / `7` / `5` |
...
审批码 `测试5`/`5` 用于洛羽-SSC主管-CN私聊里出现账号申请类关键词（外事号/外事TG号/外事邮箱/推特号/申请邮箱/注册TG/工作帐号/工作邮箱等，转发消息或直接打字均可）后：随机30-60秒自动回复"ok"；从转发来源昵称（约定格式"花名-角色-国家"，取第一段）或消息文本里的"为/给/帮XX申请"提取花名，到Drive云端 入职助手/输出 文件夹按花名反查该人的入职信息，生成【员工帐号申请】草稿发到收藏夹。关键词含"外事"归外事类目，其余默认归工作类目；SSC发送`5`后按类目放行到 `GROUP_ACCOUNT_REQUEST_FOREIGN`（测外事帐号需求群-SSC3组）或 `GROUP_ACCOUNT_REQUEST_WORK`（测工作帐号需求群-SCC3组）。当前只在测试环境配置了这两个群的真实ID（都是"测…"开头的测试群），生产环境群ID待补充，未配置前该功能自动跳过。SSC主管目前按显示名"洛羽-SSC主管-CN"匹配（未确认其Telegram用户名），后续拿到用户名可以改成更稳定的匹配方式。
