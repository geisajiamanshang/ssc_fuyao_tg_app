...
# SSC3组内部工作沟通群/转正提醒来源群出现"全员群"关键词后，把紧邻的上一条
# 图文通知转发到收藏夹，经这个审批码一次性广播到全部全员群。
ALL_STAFF_NOTICE_APPROVAL_CODE = "测试7" if ENVIRONMENT == "test" else "7"
# 洛羽-SSC主管-CN私聊里出现账号申请关键词后生成的【员工帐号申请】草稿，
# 经这个审批码放行到外事/工作两个需求群之一（按关键词是否含"外事"判断）。
ACCOUNT_REQUEST_APPROVAL_CODE = "测试5" if ENVIRONMENT == "test" else "5"
# 恒睿公司-联合管理群出现"劝退申请"/"离职申请"后，离职信息按四段分别经
# 各自审批码放行：81 离职确认转联合管理群；82 薪水结算信息私发员工本人
# 工作TG；83 离职信息同步一次性转发人事数据同步-SSC3组+北斗离职人员-
# 同步商务中心群；84 员工帐号回收转工作帐号需求群-SSC3组。四者互不影响。
OFFBOARDING_FORWARD_APPROVAL_CODE = "测试81" if ENVIRONMENT == "test" else "81"
OFFBOARDING_SALARY_APPROVAL_CODE = "测试82" if ENVIRONMENT == "test" else "82"
OFFBOARDING_SYNC_APPROVAL_CODE = "测试83" if ENVIRONMENT == "test" else "83"
OFFBOARDING_ACCOUNT_RECLAIM_APPROVAL_CODE = "测试84" if ENVIRONMENT == "test" else "84"
# 当日人事信息数据同步：SSC在自己的收藏夹发这个码触发生成（不是释放某条待
# 审批草稿，所以不放进 APPROVAL_CODES，由独立的监听器单独识别）；触发后生成
# 详细版/无详情版各一条草稿回收藏夹，91 放行无详情版到人事数据同步-SSC3组，
# 92 放行详细版到联合管理群。
...
APPROVAL_CODES = frozenset({
    ...
    ALL_STAFF_NOTICE_APPROVAL_CODE,
    ACCOUNT_REQUEST_APPROVAL_CODE,
    OFFBOARDING_FORWARD_APPROVAL_CODE,
    OFFBOARDING_SALARY_APPROVAL_CODE,
    OFFBOARDING_SYNC_APPROVAL_CODE,
    OFFBOARDING_ACCOUNT_RECLAIM_APPROVAL_CODE,
    DAILY_SYNC_SUMMARY_APPROVAL_CODE,
    DAILY_SYNC_DETAIL_APPROVAL_CODE,
})
...
# SSC3组内部工作沟通群ID尚未在该环境配置完成前，只在GROUP_REGULARIZATION_TRIGGER
# 上监控"全员群"关键词；两个来源群都没有时功能整体禁用。
ALL_STAFF_NOTICE_TRIGGER_CHAT_IDS = frozenset(
    {chat_id for chat_id in (GROUP_SSC3_INTERNAL_CHAT, GROUP_REGULARIZATION_TRIGGER) if chat_id}
)
...
OFFBOARDING_ENABLED = bool(OFFBOARDING_OUTPUT_FOLDER_ID)
...
ALLOWED_DESTINATION_IDS = frozenset({
    ...
    *([GROUP_ACCOUNT_REQUEST_FOREIGN] if GROUP_ACCOUNT_REQUEST_FOREIGN else []),
    *([GROUP_ACCOUNT_REQUEST_WORK] if GROUP_ACCOUNT_REQUEST_WORK else []),
    *([GROUP_OFFBOARDING_BUSINESS_SYNC] if GROUP_OFFBOARDING_BUSINESS_SYNC else []),
})
...
if ENVIRONMENT == "test":
    _test_chat_ids = ALLOWED_DESTINATION_IDS | {
        ...
        *([GROUP_SSC3_INTERNAL_CHAT] if GROUP_SSC3_INTERNAL_CHAT else []),
    }
